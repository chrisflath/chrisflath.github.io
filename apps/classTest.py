...
def _(get_objects, get_selection, mo, set_selection):
    ...
    return class_select, obj_names, obj_selector
...
def _(class_select, mo):
    ...
    return dim_inputs, input_height, input_radius, input_width, new_name_input, selected_class
...
def _(
    ...
    # Intro / Help Text
    intro_text = mo.md("""
    ### Willkommen im OOP Playground!
    
    Hier lernen wir den Unterschied zwischen **Klassen** und **Objekten** kennen:
    
    *   🏗️ **Klasse (Der Bauplan):** Eine Klasse (wie `Rechteck` oder `Kreis`) ist die Vorlage. Sie definiert, welche **Attribute** (z.B. Farbe, Größe) und **Methoden** (Fähigkeiten) die Dinge haben werden.
    *   📦 **Objekt (Das Ding):** Ein Objekt (wie `Tisch` oder `Schrank`) ist eine konkrete Instanz dieses Bauplans. Es hat konkrete **Attributwerte**: Der `Tisch` hat für das Attribut *Farbe* den Wert *rot*, der `Schrank` den Wert *blau*.
    
    **Probier es aus:**
    1.  Nutze den **Bauplan** oben, um neue Objekte zu erstellen.
    2.  Wähle unten ein **Objekt** aus und verändere es mit Methoden wie `anmalen` oder `vergrößern`.
    """)
    ...
    return color_btns, control_panel, create_btn, creation_panel, delete_btn, intro_text
...
def _(
    ...
    log_content = "\n".join(log_entries)
    log_view = mo.Html(f"""
    <div style="min-width: 220px; max-height: 400px; display: flex; flex-direction: column;">
        <b>Protokoll</b>
    ...
    playground = mo.hstack([
        ...
        control_panel,
        log_view
    ], align="start", gap=1, widths=[2, 1, 1])
    
    mo.vstack([
        mo.md("# Objektorientierung Playground"),
        intro_text,
        mo.md("### Klassen"),
        top_section,
        mo.md("### Objekte"),
